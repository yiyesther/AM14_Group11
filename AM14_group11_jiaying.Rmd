...
PS1_monthly <- read_excel("/Users/jyc/Desktop/finance/Assignments/PS1_Monthly.xlsx")
PS1_daily <- read_excel("/Users/jyc/Desktop/finance/Assignments/PS1_Daily.xlsx", sheet = "HPR_daily", skip = 1)
...
plot1 + labs(title = "MSFT outperformed the S&P500 and skyrocketed twice during the past 25 years",
       subtitle = "The total return index for MSFT calculated from RET and RETX, compared with that of the S&P500 index")
```

```{r plot return index GE,fig.width=10,fig.height=8,message=FALSE,warning=FALSE}
#plot total return index of GE 
GE_month %>% pivot_longer(cols = return_div:snpreturn, 
                            names_to = "total_return", 
                            values_to = "index") %>% 
  ggplot(aes(x = date, y = index, color = total_return)) +
  geom_line() +
  ...
  labs(title = "GE followed the marekt trend of S&P500 during the past 25 years",
       subtitle = "The total return index for GE calculated from RET and RETX, compared with that of the S&P500 index",
       x = "Date", y = "Compound return index") +
  scale_color_manual(name = "Return index",
                     labels = c("Total return index for RET", 
                                "Total return index for RETX", 
                                "Total return index for S&P500"),
...
monthly_simple<-MSFT_month %>% group_by(TICKER) %>% 
...
monthly_log <-MSFT_month %>% group_by(TICKER) %>% 
...
plot3 <- MSFT_month %>% pivot_longer(cols = c("RET","LRET"),
 ...
 geom_density(alpha=.2, fill="#FF6666") +
  ...
  ggtitle("Comparison of distribution of monthly simple return and log return of Microsoft")
...
plot4 + labs(title = "MSFT outperformed the S&P500 and skyrocketed twice during the past 25 years",
       subtitle = "The total return index for MSFT calculated from RET, compared with that of the S&P500 index")
```
...
#  geom_histogram(bins = 50) +
 ...
 geom_density(alpha=.2, fill="#FF6666")+
  scale_x_continuous(limits = c(-0.5,0.5)) +
  facet_wrap(~index_type, ncol = 1)+
  ggtitle("Comparison of distribution of daily simple return and log return of Microsoft")
...
plot7 <- ggplot(MSFT_day_longer,aes(x = index)) +
 geom_histogram(aes(y=..density..), colour="black", fill="white",bins = 50)+
  ...
  facet_wrap(~index_type, ncol = 1)+
  ggtitle("Comparison of distribution of daily simple return and log return of Microsoft with normal distribution")
